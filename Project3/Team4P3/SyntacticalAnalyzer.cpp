	...
	*/
	...
	if(token == NUMLIT_T || token == STRLIT_T || token == QUOTE_T){
		...
		if (token == NUMLIT_T || token == STRLIT_T){
			...
			if ( isReturned )
			{
				codeGen->WriteCode( "__RetVal = Object(" );
			}
			else
			{
...
*******************************************************************************/
int SyntacticalAnalyzer::literal (bool isReturned)
{
/*
...
*/
	if(token == IDENT_T){
		...
		string str = lex->GetLexeme();
		str = str.substr (1, str.length()-2);
//		cout << str;
		codeGen->WriteCode("\\\"" + str + "\\\"");
		...
		codeGen->WriteCode("display");
	}else if(token == NEWLINE_T){
		p2file << "Using Rule 57" << endl;
		codeGen->WriteCode("newline");

	}else if(token == LISTOP_T){
		p2file << "Using Rule 58" << endl;
		codeGen->WriteCode("listop");
		...
		codeGen->WriteCode("symbol");
		p2file << "Using Rule 64" << endl;
	}else if(token == LISTP_T){
		codeGen->WriteCode("list");
		p2file << "Using Rule 65" << endl;
	}else if(token == ZEROP_T){
		codeGen->WriteCode("zero");
